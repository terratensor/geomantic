...
func main() {
	...
	// Создаём построитель путей (без перестроения иерархии)
	builder := services.NewPathBuilder(cfg, client)

	// Запускаем построение путей
	log.Println("Starting paths building...")
	if err := builder.BuildPaths(ctx); err != nil {
		log.Fatalf("Failed to build paths: %v", err)
	}

	log.Println("Paths built successfully!")
}
